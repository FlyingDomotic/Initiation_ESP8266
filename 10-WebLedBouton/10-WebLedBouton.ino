/*
 *	Exemple de serveur Web avec gestion d'un bouton et d'une LED (10-WebLedBouton)
 *
...
void loop() {
	...
	if (clientWeb) {										// Oui, on a un client
		...
		if (requete.indexOf("/LED=OFF") >= 0 ) {			// "/LED=OFF" dans la requête ?
			#ifdef DEBUG
		...
		if (requete.indexOf("/LED=INVERSE") >= 0 ) {		// "/LED=INVERSE" dans la requête ?
			#ifdef DEBUG
		...
		clientWeb.print("<br>La LED est ");
		clientWeb.println(etatLed == ALLUME_LED ? "allumée":"éteinte");
